...
class FruitService {
    ...
    static async getFruit(name) {
        // Validator for input 'name' 
        ErrorService.validatorForGetFruitByName(name);

        // Since all keys in json are in Titlecase, Im converting the input name to titlecase
        name = name.replace(
            /\w\S*/g, (txt) => txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase()
        );

        // Return undefined if requested fruit doesnot exist 
        if(!FruitData.inventory[name]) {
          return undefined;
        }
